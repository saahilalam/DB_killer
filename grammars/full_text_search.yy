...
# Prepare 3 type of fulltext search queries condition
# Enable proximity search when its possinle to get string without quote
select:
#     natural_language_search | boolean_search | proximity_search | query_expansion_search ;
...
natural_language_search:
     ...
     nls_begin FROM _table WHERE natural_language_search_condition expression order_clause |
     nls_begin FROM _table order_clause limit_clause ;

nls_begin:
     SELECT _field_indexed[invariant],natural_language_search_condition AS SCORE ;
...
boolean_search:
     ...
     bs_begin FROM _table WHERE boolean_search_condition expression order_clause |
     bs_begin FROM _table order_clause limit_clause ;

bs_begin:
     SELECT _field_indexed[invariant],boolean_search_condition AS SCORE ;
...
query_expansion_search:
     ...
     qes_begin FROM _table WHERE query_expansion_search_condition expression order_clause |
     qes_begin FROM _table order_clause limit_clause ;

qes_begin:
     SELECT _field_indexed[invariant],query_expansion_search_condition AS SCORE ;
