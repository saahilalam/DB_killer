...
#    crashes, deadlocks, server freezes, database corruptions
# only.
#
# Recommendations:
# - concurrent.zz should be used for the generation of some initial data.
...
query_connect:
    # Do what is required for any connect/reconnect == Run the SQL from "old" query_init. But the perl code is not required.
    # As fast solution: We simple rerun 'query_init'
    query_init ;
...
init_basics2:
   # Store information if we have a debug server in $aux_file.
   ...
   # Therefore we need a $aux_file with absolute path.
   set_auxfile fill_auxfile ; SELECT CONCAT('aux_file is ->', get_auxfile , '<-') ;
set_auxfile:
   { $aux_file = $ENV{TMP} . '/rqg_' . $$ . ".aux" ; unlink($aux_file) ; return undef } ;
fill_auxfile:
   SELECT VERSION() LIKE '%debug%' INTO OUTFILE get_auxfile ;
get_auxfile:
   {return "'" . $aux_file . "'"} ;
...
is_debug1:
   ...
   {$is_debug_server = -1; open($my_file,'<'.$aux_file); read($my_file,$is_debug_server,1000); close($my_file); return undef};
...
alter_index_pk:
   ...
   # The template tables are create via the concurrency.zz grammar.
